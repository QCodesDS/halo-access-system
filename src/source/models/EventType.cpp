...
EventType parseEventType(const std::string& s) {
    if (streq(s, "LOGIN")) return LOGIN;
    if (streq(s, "LOGOUT")) return LOGOUT;
    if (streq(s, "TOKEN_REFRESH")) return TOKEN_REFRESH;
    if (streq(s, "ACCESS")) return ACCESS;
    if (streq(s, "FAILED_LOGIN")) return FAILED_LOGIN;
    if (streq(s, "OPEN_APP")) return OPEN_APP;
    if (streq(s, "DOWNLOAD")) return DOWNLOAD;
    if (streq(s, "ADMIN_ACTION")) return ADMIN_ACTION;
