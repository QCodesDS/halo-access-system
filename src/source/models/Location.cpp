...
Location parseLocation(const std::string& s) {
    if (streq(s, "US")) return US;
    if (streq(s, "VN")) return VN;
    if (streq(s, "JP")) return JP;
    if (streq(s, "KR")) return KR;
    if (streq(s, "SG")) return SG;
    if (streq(s, "CN")) return CN;
    if (streq(s, "DE")) return DE;
    if (streq(s, "FR")) return FR;
    if (streq(s, "UK")) return UK;
    if (streq(s, "AU")) return AU;
    if (streq(s, "CA")) return CA;
    if (streq(s, "IN")) return IN;
    if (streq(s, "BR")) return BR;
    if (streq(s, "RU")) return RU;
    if (streq(s, "TH")) return TH;
